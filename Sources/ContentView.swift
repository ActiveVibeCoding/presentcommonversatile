...
struct ContentView: View {
   ...
    
    ...
    func createCar() -> SCNNode {
       ...
        
        // Lights
        let lightGeo = SCNBox(width: 0.3, height: 0.1, length: 0.1, chamferRadius: 0.05)
        lightGeo.firstMaterial?.emission.contents = UIColor.yellow
        let lNode = SCNNode(geometry: lightGeo); lNode.position = SCNVector3(0.3, 0.3, 1.1)
        let rNode = SCNNode(geometry: lightGeo); rNode.position = SCNVector3(-0.3, 0.3, 1.1)
    ...
    func controlPad(label: String, color: Color = .white, active: Bool, onStart: @escaping () -> Void, onEnd: @escaping () -> Void) -> some View {
        Text(label)
            ...
            .background(active ? color.opacity(0.8) : .white.opacity(0.15))
   ...
    
    func generateGrid() -> UIImage {
        ...
        return renderer.image { ctx in
            ...
            ctx.stroke(CGRect(x: 0, y: 0, width: 64, height: 64), lineWidth: 1)
        }
    }
}
